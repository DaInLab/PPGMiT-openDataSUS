...
Escreva um programa em R que busque na web dados sobre covid-19 no Brasil e mostre gráfico de mortalidade por faixa etária.

## Resposta Bing:  

Olá, este é o Bing. Eu posso tentar te ajudar com isso. 😊
Eu encontrei alguns dados sobre covid-19 no Brasil por faixa etária no site do Ministério da Saúde[^1] e em um artigo do Poder360[^2].  
Segundo essas fontes, a faixa etária mais afetada pela doença é a de 60 anos ou mais, que representa cerca de 70% dos óbitos confirmados até junho de 2021.  
...
Eu também escrevi um programa em R que busca esses dados na web e mostra um gráfico de mortalidade por faixa etária. Você pode ver o código e o resultado abaixo:
...
grafico <- tabela %>%
  ggplot(aes(x = `Faixa Etária`, y = taxa_mortalidade)) +
  geom_col(fill = "steelblue") +
  geom_text(aes(label = round(taxa_mortalidade, 2)), vjust = -0.5) +
  labs(x = "Faixa Etária", y = "Taxa de Mortalidade (%)", title = "Mortalidade por Covid-19 no Brasil por Faixa Etária") +
  theme_minimal()
